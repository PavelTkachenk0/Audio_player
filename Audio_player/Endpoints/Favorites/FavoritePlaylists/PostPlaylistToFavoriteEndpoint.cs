...
public class PostPlaylistToFavoriteEndpoint(FavoriteService favoriteService) : EndpointWithoutRequest<FavoriteResponse>
{
    ...
    public override async Task<FavoriteResponse> ExecuteAsync(CancellationToken ct)
    {
        ...
        if (!await _favoriteService.AddPlaylistAsync(playlistId, HttpContext.User, ct))
        {
